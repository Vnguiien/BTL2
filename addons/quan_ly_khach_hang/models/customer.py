...
class Customer(models.Model):
    ...
    quotation_ids = fields.One2many('quotation', inverse_name='customer_id', string="Báo giá")
    ...
    total_quotations = fields.Integer("Tổng số báo giá", compute="_compute_total_quotations", store=True)
    ...
    # Tính toán tổng số báo giá
    @api.depends('quotation_ids')
    def _compute_total_quotations(self):
        for record in self:
    ...
    def action_view_quotations(self):
        """Xem tất cả báo giá của khách hàng"""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'Báo giá - ' + (self.customer_name or ''),
            'res_model': 'quotation',
            'view_mode': 'tree,form',
    ...
    def action_view_contracts(self):
        """Xem tất cả hợp đồng của khách hàng"""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'Hợp đồng - ' + (self.customer_name or ''),
            'res_model': 'contract',
            'view_mode': 'tree,form',
            'domain': [('customer_id', '=', self.id)],
            'context': {
    ...
    def action_view_sale_orders(self):
        ...
        return {
            'type': 'ir.actions.act_window',
            'name': 'Đơn hàng - ' + (self.customer_name or ''),
    ...
    def action_view_interactions(self):
        """Xem tất cả tương tác của khách hàng"""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': 'Tương tác - ' + (self.customer_name or ''),
            'res_model': 'crm_interact',
            'view_mode': 'tree,form',
            'domain': [('customer_id', '=', self.id)],
            'context': {
                'default_customer_id': self.id,
